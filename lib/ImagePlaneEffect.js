...
const VERT = /* glsl */ `
  ...
  uniform float uZoom;       // extra crop headroom for parallax (1 = none)
  uniform vec2 uParallax;    // uv offset of the moving crop (parallax)

  varying vec2 vUv;

  void main() {
    // Cover-fit: crop the texture uv so a mismatched image ratio fills the plane
    // without stretching (identity 1,1 when the plane matches the image). uZoom
    // crops in a touch further to leave headroom, and uParallax slides that crop
    // window — so the image shifts inside its fixed frame (parallax) with no
    // plane movement and no spill over neighbours.
    vUv = (uv - 0.5) * (uCoverScale / uZoom) + 0.5 + uParallax;

    vec2 p = position.xy + 0.5;                // 0..1
    float curve = sin(p.x * 3.14159265) * uScrollVelo * 6.0; // px
...
export default class ImagePlaneEffect {
	constructor(container, options = {}) {
		...
		// Easing + limits. Kept tame — the effect should read as a subtle shimmer.
		this.ease = options.ease ?? 0.1;
		this.maxVelo = options.maxVelo ?? 0.02;
		// Strength of the (ungated) scroll-driven distortion.
		this.scrollGain = options.scrollGain ?? 0.5;
		// Cursor-follow aberration lens. false = ignore the mouse entirely (only
		// the scroll-driven distortion remains).
		this.hover = options.hover ?? true;
		// Displacement axis: [1,0] horizontal (carousel), [0,1] vertical (gallery).
		this.dir = options.dir ?? [1, 0];
		// Parallax: how far (in uv) the image crop slides as a plane travels from
		// one edge of the viewport to the other, along `dir`. 0 disables it (the
		// marquee). `parallaxZoom` crops in to leave headroom so the slide never
		// exposes an edge; derived from the amount unless overridden.
		this.parallax = options.parallax ?? 0;
		this.parallaxZoom = options.parallaxZoom ?? (this.parallax > 0 ? 1 + this.parallax * 2.4 : 1);
		// Per-image film-grain strength (there's also a global GrainOverlay).
		this.grain = options.grain ?? 0.03;
		// RGB channel-split strength (chromatic aberration), scaled by velocity in
		// the shader so it's crisp at rest. Keep it slight — this is a fringe, not
		// a glitch. 0 disables it.
		this.chroma = options.chroma ?? 0.6;
		// Optional external velocity source: a function returning a signed,
		// already-normalised velocity (roughly matching scroll's units) that is
		// added to the scroll-driven distortion. Lets a caller whose motion the
		// effect can't see — e.g. the marquee's own drift/boost — drive the bend
		// and aberration. Returns 0 / omitted for scroll+hover only.
		this.getExternalVelo = options.getExternalVelo ?? null;
		// Device-pixel-ratio cap. 1 keeps fill-rate low on HiDPI/old GPUs; the
		// grain and aberration hide the softness.
		...
		this.targetSpeed = 0;
		this.scrollVelo = 0;
		this.sharedScrollVelocity = 0;
		this.amount = 0; // grain seed / time
		this.raf = 0;
		this.visible = false; // driven by IntersectionObserver — render only in view
		this.canvasRect = null; // cached; refreshed on scroll/resize
		this.disposed = false;
	...
	_init() {
		this.renderer = new Renderer({
			...
			dpr: Math.min(window.devicePixelRatio || 1, this.dpr),
		});
		...
		// The canvas rect only changes on scroll/resize — cache it from the
		// shared scroll frame instead of reading it every render.
		this._unsubscribeScrollFrame = onScrollFrame((_, frameVelocity) => {
			this.sharedScrollVelocity = frameVelocity;
		...
		// Only run the render loop while the section is on (or near) screen.
		this._io = new IntersectionObserver(
			(entries) => {
				this.visible = entries[0].isIntersecting;
				if (this.visible && !this.raf && !this.disposed) {
	...
	_updateSpeed() {
		...
		const raw = clamp(Math.abs(this.sharedScrollVelocity) / window.innerHeight, 0, this.maxVelo);
		this.scrollVelo -= this.ease * (this.scrollVelo - raw);
	}

	_loop() {
		if (this.disposed) return;
		// Off-screen: stop the loop entirely; the IntersectionObserver restarts it.
		if (!this.visible) {
			this.raf = 0;
			return;
		}
		...
		// Mouse speed drives the (circle-gated) hover aberration; scroll velocity
		// drives an ungated distortion in the shader — see uScrollVelo below.
		const mouseVelo = this.hover ? clamp(this.targetSpeed, 0, this.maxVelo) : 0;
		// Scroll velocity plus any external drift (e.g. the marquee's own speed),
		// which is already signed/normalised by the caller.
		const externalVelo = this.getExternalVelo ? this.getExternalVelo() : 0;
		const scrollVelo = this.scrollVelo * this.scrollGain + externalVelo;
		...
		for (const plane of this.planes) {
			...
			// Parallax: slide the crop opposite to the plane's distance from the
			// viewport centre along the travel axis, so images drift within their
			// frames as the carousel moves. Clamped to the crop headroom (set by
			// uZoom) so an edge is never exposed.
			if (this.parallax > 0) {
				const vw = window.innerWidth || 1;
				const vh = window.innerHeight || 1;
				const rel = this.dir[0]
					? clamp((r.left + r.width / 2 - vw / 2) / vw, -1, 1)
					: clamp((r.top + r.height / 2 - vh / 2) / vh, -1, 1);
				const headX = Math.max(0, (1 - sx / this.parallaxZoom) / 2);
				const headY = Math.max(0, (1 - sy / this.parallaxZoom) / 2);
				u.uParallax.value = [
					this.dir[0] ? clamp(rel * this.parallax, -headX, headX) : 0,
					this.dir[1] ? clamp(rel * this.parallax, -headY, headY) : 0,
				];
			}
			...
			u.uMouse.value = this.hover ? [(mu - 0.5) * sx + 0.5, (mv - 0.5) * sy + 0.5] : [-1, -1];
			u.uVelo.value = mouseVelo;
			u.uScrollVelo.value = scrollVelo;
	...
	dispose() {
		...
		if (this._unsubscribeScrollFrame) this._unsubscribeScrollFrame();
